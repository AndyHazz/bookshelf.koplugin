...
**Also show status line in reader.** Directly under the Status line entry is a switch that mirrors the same line across the top of the reader, using the [Bookends](https://github.com/AndyHazz/bookends.koplugin) plugin. It uses the same text, font, size, weight and alignment, so the strip does not change as you move between the shelf and a book. Edit the line here and the reader follows. Needs Bookends installed; harmless if not.
